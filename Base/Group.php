...
class Group {
	...
	public static function NewGroup($key, $title, $param, $operator, $value, $hide = false){
		$group = new Group($key, $title, false, false);
		$group->location($param, $operator, $value);
		$group->_build();
	}

	public function __construct( $key, $title, $hide = false, $option_page = false ) {
		$this->args['key']   = $key;
